...
// Reinput data
  ...
  it('Edit User Detail', () => {
    ...
    cy.wait(5000);
    cy.get(':nth-child(3) > .ant-card > .ant-card-body > :nth-child(1) > .ant-form > :nth-child(1) > .simple-form-item-value').scrollTo('center',{ensureScrollable: true})
    
    cy.get('.anticon-delete').eq(0).click();
    cy.get('.ant-btn-dangerous').contains('Remove').click({force: true});
    cy.get('.anticon-delete').eq(1).click()
    cy.get('.ant-btn-dangerous').contains('Remove').click({force: true});
    cy.get('.anticon-delete').eq(2).click({force: true})
    cy.get('.ant-btn-dangerous').contains('Remove').click({force: true});
 

    cy.get('.anticon-edit').eq(2).click()
    cy.wait(5000);
    cy.get('.ant-upload-list-item-card-actions-btn').eq(1).click()
    cy.get('.ant-btn-dangerous').contains('Remove').click();
