...
// Check text
  it('Check text', () => {
    ...
    cy.get('.ant-card-head-title').eq(1).should('have.text','Profile Completion')
    cy.get('.profile-completion-section-container').should('contain','Fill out your profile details to enjoy a better experience!');
    cy.get('.ant-card-head-title').eq(2).should('have.text','Evaluation Levels')
    cy.get('.certification-level-section-row').eq(0).should('contain','Basic')
                                                              .and('contain','Basic means that the vendor makes a public statement of conformity with the security requirements');
    cy.get('.certification-level-section-row').eq(1).should('contain','Enhanced')
                                                              .and('contain','Enhanced means that the vendor delivers the evidence that the product conforms to the security requirements - an independent accredited or notified third-party, not under control of the vendor, performs an assessment of the security based on a review of the technical documentation. This level provides assurance in that the product have been evaluated at a level intended to minimize the known basic risks of cyber-incidents and cyber-attacks.');
    cy.get('.certification-level-section-row').eq(2).should('contain','Substantial')
                                                              .and('contain','Substantial means that the vendor delivers the product itself along with evidence that it conforms to the security requirements - an independent accredited or notified third-party, not under control of the vendor, performs an assessment of the security based on at least: a review to demonstrate the absence of known vulnerabilities; testing to demonstrate that the product correctly implement the security functionalities. This level provides assurance in that the product have been evaluated at a level intended to minimize cybersecurity risks, cyber-incidents and cyber-attacks carried out by actors with limited skills and resources.');
    cy.get('.certification-level-section-row').eq(3).should('contain','High')
                                                              .and('contain','High means that the vendor delivers the product itself along with evidence that it conforms to the security requirements - an independent accredited or notified third-party, not under control of the vendor, performs an assessment of the security based on at least: a review to demonstrate the absence of known vulnerabilities;testing to demonstrate that the product correctly implement the security functionalities; an assessment of their resistance to skilled attackers using penetration testing.This level provides assurance in that the product have been evaluated at a level intended to minimize the risk of state-of-the-art cyber-attacks carried out by actors with significant skills and resources.');
  ...
  it('Click on Launch Knowledge-Base button ', () => {
    ...
    cy.url().should('eq','https://app.develop.cyber-pass.eu/dashboard/coming-soon');
  ...
  it('Click on Your Certified Products button ', () => {
    ...
    cy.url().should('eq','https://app.develop.cyber-pass.eu/certified-products/list');
